...
if (-not $SkipHealthCheck) {
    ...
    if (Test-Path ".\azure-health-check.ps1") {
        ...
        Write-Host "  ⚠ Health check script not found, skipping..." -ForegroundColor Gray
    }
} else {
...
if (-not (Test-Path ".\publish\functions.metadata")) {
    Write-Host "  ✗ functions.metadata not found in publish folder!" -ForegroundColor Red
...
Write-Host "  ✓ functions.metadata found ($metadataSize bytes)" -ForegroundColor Green
...
Write-Host "  ✓ $functionCount function(s) defined:" -ForegroundColor Green
...
if ($discoveredFunctions -match "ArchiveFunction" -and 
    ...
    Write-Host "`n✓ SUCCESS: All 3 functions discovered!" -ForegroundColor Green
    ...
    Write-Host "`n⚠ WARNING: Functions may not have been discovered correctly." -ForegroundColor Yellow
